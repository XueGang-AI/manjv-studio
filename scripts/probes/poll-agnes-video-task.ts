// ============================================
// 轮询指定 Agnes 视频任务（支持长时间等待）
// 用法: npx tsx scripts/probes/poll-agnes-video-task.ts --task-id <id> [--timeout-minutes 60] [--interval-seconds 10]
// ============================================
...
function parseArgs(): { taskId: string; timeoutMinutes: number; intervalSeconds: number } {
  ...
  if (!taskId) {
    ...
    console.error('用法: npx tsx scripts/probes/poll-agnes-video-task.ts --task-id <id> --timeout-minutes 60 --interval-seconds 10')
    process.exit(1)
  }
  return {
    taskId,
    timeoutMinutes: parseInt(getArg('timeout-minutes', '60'), 10),
...
async function main() {
  ...
  console.log('🔍 Agnes Video Task Poller\n')
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      if (status === 'completed' || status === 'succeeded' || status === 'success') {
        ...
        fs.writeFileSync(
          `${outputDir}/video-task-completed-${taskId}.json`,
      ...
      if (status === 'failed' || status === 'error') {
        ...
        fs.writeFileSync(
          `${outputDir}/video-task-failed-${taskId}.json`,
  ...
  console.log()
  fs.writeFileSync(
    `${outputDir}/video-task-timeout-${taskId}.json`,
