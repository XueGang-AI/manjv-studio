...
- 2026-06-28 已完成 90 秒《古城最后一盏花灯》真实 API MP4 QA，当前 Medium Agent Plan 视频默认使用 `doubao-seedance-1.5-pro`；Seedance 2.0 仅作为高套餐或开通后可选能力。
...
- 多人镜头不明显串脸。
- 分镜图生成请求包含匹配角色的 `referenceImages`；视频阶段使用已确认分镜图作为首帧，继承首帧内已经固化的人物外观。

场景一致性：

- 同一地点复用同一组 `SceneImage`。
- 镜头 Prompt 保留场景文字描述，分镜图生成同时传入场景参考图。
- 视频片段继承已确认分镜图首帧中的场景布局、灯光和色调。
