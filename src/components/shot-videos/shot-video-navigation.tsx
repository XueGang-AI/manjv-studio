...
import { Check, Clock, Video, Loader2, AlertCircle, Filter } from 'lucide-react'
import { cn } from '@/lib/utils'
import { Badge } from '@/components/ui/badge'
import { getVideoGroupStatus, STATUS_LABELS, type ShotVideoGroup, type VideoStatus } from './shot-videos-types'

interface ShotVideoNavigationProps {
...
const STATUS_ICON: Record<string, React.ReactNode> = {
  ...
  timeout: <Clock size={10} className="text-[var(--color-warning)]" />,
  none: <Video size={10} className="text-[var(--color-text-muted)]" />,
}

const FILTER_OPTIONS: Array<{ value: VideoStatus | 'all'; label: string }> = [
  { value: 'all', label: '全部' },
...
export function ShotVideoNavigation({ shots, isGenerating, activeShotId, onSelect }: ShotVideoNavigationProps) {
  ...
  const filteredShots = statusFilter === 'all'
    ? shots
    : shots.filter(s => getVideoGroupStatus(s, isGenerating) === statusFilter)
