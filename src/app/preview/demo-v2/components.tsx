/**
 * Manjv Studio V2 — 共享 UI 组件库
 * 风格：AI 创作平台 / 科技 SaaS / Indigo+Cyan 双色调
 */
...
// Button
// ============================================
type BtnV = 'primary' | 'secondary' | 'ghost' | 'danger' | 'cyan'
type BtnS = 'sm' | 'md' | 'lg'

const BTN_V: Record<BtnV, string> = {
  primary: 'bg-[var(--primary)] text-white hover:bg-[var(--primary-hover)] shadow-[var(--glow-primary)]',
...
export function ProgressBar({ value = 0, variant = 'primary', size = 'sm' }: {
  value: number; variant?: 'primary' | 'cyan' | 'success' | 'warning'; size?: 'sm' | 'md'
}) {
  const c = { primary: 'bg-[var(--primary)]', cyan: 'bg-[var(--accent-cyan)]', success: 'bg-[var(--success)]', warning: 'bg-[var(--warning)]' }
  const h = size === 'sm' ? 'h-1.5' : 'h-2.5'
  return (
    <div className={`w-full ${h} bg-[var(--bg-panel)] rounded-full overflow-hidden`}>
      <div className={`${h} rounded-full transition-all duration-500 ease-out ${c[variant]}`}
...
export function AISuggestionCard({ title, desc, action, variant = 'primary' }: {
  ...
  const colors = {
    primary: 'border-[var(--primary)]/20 bg-[var(--primary-muted)]',
    cyan: 'border-[var(--accent-cyan)]/20 bg-[var(--accent-cyan-muted)]',
    warning: 'border-[var(--warning)]/20 bg-[var(--warning-muted)]',
  }
  const iconColors = {
  ...
  return (
    <div className={`border rounded-[var(--radius-md)] p-3.5 ${colors[variant]}`}>
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-3 rounded-[var(--radius-md)] hover:bg-[var(--bg-panel)] transition-colors">
      ...
      {status === 'generating' && progress !== undefined && (
        <div className="w-16">
          <ProgressBar value={progress} variant="cyan" size="sm" />
        </div>
      )}
    ...
    <div className="flex items-center gap-1 bg-[var(--bg-panel)] border border-[var(--border-dim)] rounded-[var(--radius-md)] p-1">
      {(['agnes', 'ark'] as const).map(m => (
        <button key={m} className={`
          ...
          ${active === m ? 'bg-[var(--primary)] text-white shadow-[var(--glow-primary)]' : 'text-[var(--text-muted)] hover:text-[var(--text-secondary)]'}
        `}>
          {m === 'agnes' ? 'Agnes' : '豆包'}
        </button>
      ))}
...
export function NotificationBell({ count = 0 }: { count?: number }) {
  return (
    <button className="relative p-2 rounded-[var(--radius-md)] hover:bg-[var(--bg-panel)] transition-colors text-[var(--text-muted)] hover:text-[var(--text-secondary)] cursor-pointer">
      ...
      {count > 0 && (
        <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-[var(--error)] text-white text-[9px] font-bold rounded-full flex items-center justify-center">
          {count}
        </span>
      )}
    </button>
...
export function UserMenu() {
  return (
    <div className="flex items-center gap-2.5 cursor-pointer hover:bg-[var(--bg-panel)] rounded-[var(--radius-md)] px-2 py-1.5 transition-colors">
      <div className="w-7 h-7 rounded-full bg-[var(--gradient-primary)] flex items-center justify-center text-white text-xs font-bold">X</div>
