/**
 * Manjv Studio V2 — 项目列表 / 工作台首页
 *
 * 成品级 AI 创作平台首页：
 * - 顶部产品 Header（搜索、通知、用户、新建）
 * - 统计概览 + 资源用量
 * - AI 智能推荐区
 * - 项目卡片（封面、状态、进度、快捷操作）
 * - 最近活动 Timeline
...
import {
  Plus, Search, Bell, Film, Clock, Users, Image, Video,
  Clapperboard, ArrowRight, Sparkles, ChevronRight, Grid3X3,
  List, MoreHorizontal, Copy, Trash2, Download, Eye, Play,
  FolderOpen, TrendingUp, Zap, AlertTriangle, RotateCcw,
  LayoutDashboard, Settings, FileCode, ListTodo, Layers,
  Command, Upload, LayoutTemplate, Package,
} from 'lucide-react'
import {
...
// ===== Mock Data =====
const MOCK_PROJECTS = [
  { id: '1', name: '都市雨夜', types: ['都市','悬疑'], styles: ['韩漫','电影感'], status: '视频生成中', statusV: 'info' as const, step: 7, totalSteps: 8, provider: 'agnes', duration: 60, updated: '2 小时前', assets: 18, cover: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)' },
  { id: '2', name: '仙途奇缘', types: ['仙侠','冒险'], styles: ['国漫','水墨'], status: '分镜待确认', statusV: 'warning' as const, step: 5, totalSteps: 8, provider: 'ark', duration: 90, updated: '昨天', assets: 24, cover: 'linear-gradient(135deg, #1a0533 0%, #2d1b69 50%, #11001c 100%)' },
  { id: '3', name: '重生之巅', types: ['都市','逆袭'], styles: ['韩漫'], status: '角色图已确认', statusV: 'success' as const, step: 4, totalSteps: 8, provider: 'agnes', duration: 30, updated: '3 天前', assets: 12, cover: 'linear-gradient(135deg, #0d1117 0%, #1c2333 50%, #2d333b 100%)' },
  { id: '4', name: '星际迷途', types: ['科幻','惊悚'], styles: ['赛博朋克'], status: '成片已生成', statusV: 'cyan' as const, step: 8, totalSteps: 8, provider: 'ark', duration: 120, updated: '1 周前', assets: 35, cover: 'linear-gradient(135deg, #0a0e27 0%, #1a0a3e 50%, #0d001a 100%)' },
  { id: '5', name: '校园心跳', types: ['校园','甜宠'], styles: ['日漫'], status: '草稿', statusV: 'default' as const, step: 0, totalSteps: 8, provider: 'agnes', duration: 15, updated: '2 周前', assets: 0, cover: 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)' },
]
  ...
  return (
    <header className="h-14 border-b border-[var(--border-dim)] bg-[var(--bg-surface)]/80 backdrop-blur-md flex items-center px-5 gap-4 sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2.5 mr-4">
        <div className="w-8 h-8 rounded-[var(--radius-md)] flex items-center justify-center" style={{ background: 'var(--gradient-primary)' }}>
      ...
      <div className="flex-1 max-w-md">
        <button onClick={() => setSearchOpen(!searchOpen)}
          className="w-full h-9 flex items-center gap-2 px-3 bg-[var(--bg-elevated)] border border-[var(--border-dim)] rounded-[var(--radius-md)] text-sm text-[var(--text-muted)] hover:border-[var(--border-bright)] transition-colors cursor-pointer">
          <Search size={14} />
      ...
      <div className="ml-auto flex items-center gap-1">
        <Button variant="primary" size="sm" icon={<Plus size={14} />}>新建项目</Button>
...
function QuickActions() {
  const items = [
    { icon: <Plus size={18} />, label: '新建漫剧', desc: '从零开始创作', variant: 'primary' as const },
    { icon: <LayoutTemplate size={18} />, label: '从模板创建', desc: '热门风格模板', variant: 'secondary' as const },
    { icon: <Upload size={18} />, label: '导入脚本', desc: '小说/剧本一键导入', variant: 'secondary' as const },
    { icon: <Package size={18} />, label: '素材库', desc: '管理图片视频', variant: 'secondary' as const },
  ]
  return (
      ...
      {items.map(item => (
        <Card key={item.label} hover className="p-4 cursor-pointer group">
          <div className={`w-10 h-10 rounded-[var(--radius-md)] flex items-center justify-center mb-3 transition-colors ${
            item.variant === 'primary' ? 'bg-[var(--primary-muted)] text-[var(--primary)] group-hover:bg-[var(--primary)] group-hover:text-white' : 'bg-[var(--bg-panel)] text-[var(--text-muted)] group-hover:text-[var(--text-secondary)]'
          }`}>
            {item.icon}
          </div>
...
function AIRecommendations() {
  return (
    <div className="space-y-3">
      <AISuggestionCard
        title="继续创作「都市雨夜」"
        desc="上次停在视频生成阶段，3 个镜头仍在生成中。"
        action={<Button variant="cyan" size="sm" icon={<Play size={12} />}>继续创作</Button>}
        variant="cyan"
      />
      <AISuggestionCard
        title="「仙途奇缘」有 1 个失败任务"
        desc="第 7 镜头视频生成失败，建议重试或切换模型。"
        action={<Button variant="secondary" size="sm" icon={<RotateCcw size={12} />}>一键重试</Button>}
        variant="warning"
      />
      <AISuggestionCard
        title="推荐尝试豆包模型"
        desc="角色一致性和视频质量优于 Agnes，首月赠送 500 积分。"
        action={<Button variant="secondary" size="sm">了解详情</Button>}
      />
    </div>
  )
}
  ...
  return (
    <Card hover className="overflow-hidden group">
      ...
      <div className="h-36 relative overflow-hidden" style={{ background: p.cover }}>
        ...
        <div className="absolute left-3 top-3 bottom-3 flex flex-col gap-1.5 opacity-30">
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
          <div className="flex gap-2">
            <Button variant="primary" size="sm" icon={<Play size={12} />}>继续创作</Button>
      ...
      <div className="p-4 space-y-3">
        ...
        <div>
          ...
          <ProgressBar value={(p.step / p.totalSteps) * 100} variant={p.step === p.totalSteps ? 'success' : 'primary'} />
        </div>
        {/* Footer */}
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-7 w-36 bg-[var(--bg-panel)] border border-[var(--border-default)] rounded-[var(--radius-md)] shadow-[var(--shadow-elevated)] py-1 z-10">
                {[
                  { icon: <Copy size={12} />, label: '复制项目' },
                  { icon: <Download size={12} />, label: '导出' },
                  { icon: <Trash2 size={12} />, label: '删除', danger: true },
                ].map(item => (
                  <button key={item.label} className={`w-full flex items-center gap-2 px-3 py-2 text-xs cursor-pointer transition-colors ${item.danger ? 'text-[var(--error)] hover:bg-[var(--error-muted)]' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-elevated)]'}`}>
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[var(--bg-base)]">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-[1400px] mx-auto px-6 py-6 space-y-8">
          ...
          {/* Row 2: AI Recommendations + Quick Actions */}
          <div className="grid grid-cols-5 gap-6">
            <div className="col-span-3 space-y-4">
              <h2 className="text-sm font-semibold text-[var(--text-primary)] flex items-center gap-2">
                <Sparkles size={14} className="text-[var(--primary)]" /> AI 建议
              </h2>
              <AIRecommendations />
            </div>
          ...
          <div>
                ...
                <div className="flex items-center gap-1 bg-[var(--bg-elevated)] border border-[var(--border-dim)] rounded-[var(--radius-md)] p-1">
                  {[{ k: 'all', l: '全部' }, { k: 'active', l: '进行中' }, { k: 'completed', l: '已完成' }, { k: 'draft', l: '草稿' }].map(f => (
                    <button key={f.k} onClick={() => setFilter(f.k)}
                      className={`px-3 py-1.5 rounded-[var(--radius-sm)] text-xs font-medium transition-all cursor-pointer ${filter === f.k ? 'bg-[var(--primary)] text-white' : 'text-[var(--text-muted)] hover:text-[var(--text-secondary)]'}`}>
                      {f.l}
                    </button>
                  ))}
                </div>
            ...
            {filtered.length === 0 ? (
              <EmptyState icon={<FolderOpen size={28} />} title="没有匹配的项目" desc="调整筛选条件，或创建新项目开始创作" action={<Button variant="primary" size="sm" icon={<Plus size={14} />}>新建项目</Button>} />
            ) : viewMode === 'grid' ? (
                  ...
                  <Card key={p.id} hover className="cursor-pointer">
                    <div className="flex items-center gap-4 px-5 py-4">
                      ...
                      <div className="w-32 shrink-0">
                        <ProgressBar value={(p.step / p.totalSteps) * 100} />
          ...
          {/* Row 4: Activity + Generation Queue */}
          <div className="grid grid-cols-5 gap-6">
